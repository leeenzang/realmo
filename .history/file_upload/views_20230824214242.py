...
def upload_csv(request):
    ...
    daily_total_visitors = Visitor.objects.filter(사용일__gte=three_months_ago).dates('사용일', 'day').annotate(count=Count('UID')).order_by('-사용일')

    # 주어진 데이터와 함께 템플릿에 렌더링해서 사용자에게 웹 표시
    return render(request, 'file_upload/upload.html', {'form': form, 'message': message, 'daily_visitors': daily_visitors_count})
